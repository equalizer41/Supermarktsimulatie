    ...
    public Grid(int width, int height, int cellSize) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                tiles[x][y] = new Tile(true, Color.LIGHTGRAY, null,1,1);
            }
        }
    }
        ...
        for (int dx = 0; dx < widthInCells; dx++) {
            for (int dy = 0; dy < heightInCells; dy++) {
                ...
                if (dx == 0 && dy == 0) {
                    ...
                    Tile placeholder = new Tile(true, currentFloor.getColor(), floorImage, 1, 1);
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                // Anders teken de normale tile image (voor reguliere 1x1 floors/walls)
                    ...
                    } else if (tile.getColor() != Color.TRANSPARENT) {
